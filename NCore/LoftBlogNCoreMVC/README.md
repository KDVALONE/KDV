...
- Используем паттерн реопзиторий (есть интерфейсы, и есть их реализация), это позволит создать контейнеры для DI и обособить их от реализации
а управлять ими будет прослойка класс DataManager(херовое название Макконел(с))
Логика в следующем, в interface должен реализовывать возможность, взять директорию, взять список директорий, сохранить/удалить директорию. Тоже и для Materials

Создаем классы реализующие интерфейсы